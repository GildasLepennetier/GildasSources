...
# pRESTO 
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
if [ ];then echo "extract FASTQ archive: .gz -> .fastq  !! NEED TO BE fastq for AssemblePairs !!"
	zcat $READ1_GZ > $READ1
	zcat $READ2_GZ > $READ2
fi
...
if [ ]; then echo "using the whole file - for pRESTO igblast_out_changeo   ->   -outfmt '7 std qseq sseq btop' "
	...
	#-evalue 1.0e-10 #(1.0e-10 to avoid excessive size of file and low quality matches)
...
if [ 0 ];then echo "IgTree / ClustalW"
	...
	if [ 0 ];then echo "ClustalW"
		...
		#take each group and align it using clustalw
		...
		ls *.fa | time parallel --eta clustalw -quiet -align -output=pir -infile={}
		
		for File in $(ls *.pir); do
			cat $File | sed "s/DL;M04284_13_000000000-ATVWH_//g" > $File.short
		done
	...
	if [ 0 ];then echo "IgTree making the tree"
		...
		for File in $(ls Clones/*.short); do
